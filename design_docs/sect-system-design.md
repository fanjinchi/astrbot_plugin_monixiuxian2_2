...
**传承系统改版联动（2026-08-24 补记，v3.11.0）**

7. **宗门传承实例**：宗门宝库新增 `legacies` 列表条目（`kind="legacy"`），每人限领一条本宗传承实例；领取需先通过守护 NPC 挑战（复用 PvE 战斗，失败不占名额可重试）；实例绑定宗门（`legacy_instances.sect_id`），离宗/被踢自动回收（`delete_legacy_instances_by_owner_sect`）；**宗门传承不可被 PK 夺取**（`select_snatch_target` 跳过 sect 类型）。
8. **术语消歧**：本文档 "传承（功法/心法）" 指宗门功法的固有属性（只可传予本宗之人），与新传承系统中的 "宗门传承实例"（宝库 kind=legacy 奖励包）是不同事物；后者语境限定为 "个人传承"。
