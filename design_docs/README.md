...
| `season-1-outline.md` | 本项目 | **第一季剧情大纲**（2026-09-03 建立，openspec change `narrative-content-pipeline`）：剧情幕表（练气/筑基/金丹/元婴 × 复苏阶段/妖乱强度/秘境开启叙事/宗门对散修态度）、第一季内容预算表、四梯队填充顺序计划（标注每梯队工程前置依赖）。季度层增量大纲，随每季重写（第二季另建 `season-2-outline.md`）；恒常主线轴见 `world-bible.md` §1.1。 |
