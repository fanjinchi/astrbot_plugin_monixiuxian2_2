...
| `content-design/` | 本项目 | 玩家侧内容设计工作区（武器/功法/心法定稿数值，最终产出 `config/weapons.json` / `skills.json` / `heart_methods.json`）；设计蓝本取自 `mybrute/`、`qpet-daledou/` 外部调研。**双路线定位与内容三族/机制预算规范：`content-design/route-identity.md`（2026-08-27 定稿，openspec change `dual-route-identity`），填充内容前必读。** 另有叙事 canon 体系（`weapons/skills/heart_methods` 三表增 `canon_origin/tone_tier/story_hook/narrative_status` 四列；`events-canon.csv` / `enemies-canon.csv` / `rifts-canon.csv` / `bounty-canon.csv` 四张轻量叙事表）、叙事 lint 脚本 `lint_narrative.py`（禁词/数值承诺/长度/品级冠词/名字一致性闸门，`sync_content_to_config.py` 预算验算后的第二道闸门）与逆向导出补登记脚本 `../scripts/export_config_to_canon.py`（config 缺失条目以 legacy 行补登进设计表，防 reconcile 误删）。 |
