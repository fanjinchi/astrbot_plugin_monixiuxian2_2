...
| `world-bible.md` | 本项目 | **世界观设定集（World Bible）**（2026-08-24 建立，openspec change `worldview-bible`；2026-08-26 设定修订：六州山河州域正式化、秘境示例化、五宗规划+合欢宗延后、心法文风按宗门形象分档、废除体修短寿叙事；2026-09-03 叙事内容管线增补：§6 全枚举退役迁 canon 表、§1.3b 擦边尺度、§1.6 文案载体清单、§1.7 变体量标准、§3.2.6 NPC 三件套、§3.6 势力冲突矩阵；2026-08-30 文案外移落地：§1.6 清单物理位置改为 config 路径（narrative_config 五节/spirit_root_descriptions/rift description 与 explore_events/adventure 分桶变体池））：摸鱼修仙界主线性（灵气枯竭→复苏）、基调与玩梗/擦边尺度、地理（六州山河+妖域四层）、势力（五宗规划+合欢宗延后+冲突矩阵）、修炼体系叙事化（灵/体双词汇表）、命名规范（品级词+禁用词）、既有内容收编清单（全枚举已迁 canon 表）。**全部内容文案的唯一事实来源**。世界观/文案基调变更时须同步本文。 |
| `season-1-outline.md` | 本项目 | **第一季剧情大纲**（2026-09-03 建立，openspec change `narrative-content-pipeline`）：剧情幕表（练气/筑基/金丹/元婴 × 复苏阶段/妖乱强度/宗门对散修态度）、第一季内容预算表、四梯队填充顺序计划（标注每梯队工程前置依赖）。季度层增量大纲，随每季重写（第二季另建 `season-2-outline.md`）；恒常主线轴见 `world-bible.md` §1.1。秘境条目已移出大纲（说明后置至 bd `trl`）。 |
| `剧情/` | 本项目 | **tier-1 高频文案创作工作区**（2026 建立，openspec change `season-1-tier1-copywriting`）：`00-剧情总骨架.md`=四域（突破/修炼/战斗/事件）场景×分支矩阵、双路线词汇表、段位节拍、六州风味表、NPC 弹药表、变量白名单与样张；`01~09` 剧本册按骨架成文（**9 册共 341 条**：册 1-5 散修 15 事件 165 条 [11 散修+青云门 4]、册 6-9 四宗 176 条），**两轮全量审阅修复已闭环**，按册校验后灌入 `content-design/copy_variants.csv`；文风按 bible §1.3（正经+冷幽默正/冷各半进正文、括号彩蛋位专属玩梗灰）；写作素材取自 `novel-research/`；散修前辈相助不含宗门 NPC，宗门 NPC 只出宗门专属事件。 |
...
| `content-design/` | 本项目 | 玩家侧内容设计工作区（武器/功法/心法/防具定稿数值，最终产出 `config/weapons.json` / `skills.json` / `heart_methods.json` / `items.json` 防具段）；设计蓝本取自 `mybrute/`、`qpet-daledou/` 外部调研。**双路线定位与内容四族/机制预算规范：`content-design/route-identity.md`（2026-08-27 定稿，openspec change `dual-route-identity`；2026-09-08 满级面板拆分天生/装备护甲），填充内容前必读。** 防具：`armors.csv` + `armors.md`（2026-09-08 `armor-content-design`，对称 EHP 框架：重甲/法袍 × 9 品级 18 标杆件全通用，占位 3 件删除留档，待用户确认后导入）。另有叙事 canon 体系（`weapons/skills/heart_methods` 三表增 `canon_origin/tone_tier/story_hook/narrative_status` 四列；`events-canon.csv` / `enemies-canon.csv` / `rifts-canon.csv` / `bounty-canon.csv` 四张轻量叙事表）、叙事 lint 脚本 `lint_narrative.py`（禁词/数值承诺/长度/品级冠词/名字一致性闸门，`sync_content_to_config.py` 预算验算后的第二道闸门）与逆向导出补登记脚本 `../scripts/export_config_to_canon.py`（config 缺失条目以 legacy 行补登进设计表，防 reconcile 误删）。 |
