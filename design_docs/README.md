...
| `world-bible.md` | 本项目 | **世界观设定集（World Bible）**（2026-08-24 建立，openspec change `worldview-bible`；2026-08-26 设定修订：六州山河州域正式化、秘境示例化、五宗规划+合欢宗延后、心法文风按宗门形象分档、废除体修短寿叙事；纯设定修订不改 config）：摸鱼修仙界主线性（灵气枯竭→复苏）、基调与玩梗尺度（合格/不合格对照）、地理（六州山河+妖域四层）、势力（五宗规划+合欢宗延后+宗门-路线绑定规则）、修炼体系叙事化（灵/体双词汇表）、命名规范（品级词+禁用词）、既有内容收编清单。**全部内容文案的唯一事实来源**。世界观/文案基调变更时须同步本文。 |
...
| `content-design/` | 本项目 | 玩家侧内容设计工作区（武器/功法/心法定稿数值，最终产出 `config/weapons.json` / `skills.json` / `heart_methods.json`）；设计蓝本取自 `mybrute/`、`qpet-daledou/` 外部调研。**双路线定位与内容三族/机制预算规范：`content-design/route-identity.md`（2026-08-27 定稿，openspec change `dual-route-identity`），填充内容前必读。** |
