...
class BankHandlers:
    ...
    async def handle_bank_info(self, player: Player, event: AstrMessageEvent):
        ...
        if info.get("loan"):
            ...
            if loan_info:
                loan_type_name = (
                    "突破贷款"
                    if loan_info["loan_type"] == "breakthrough"
                    else "普通贷款"
                )
                status = (
                    "⚠️ 已逾期！"
                    if loan_info["is_overdue"]
                    else f"剩余 {loan_info['days_remaining']} 天"
                )
                msg_lines.extend(
                    [
                        "━━━━━━━━━━━━━━━",
    ...
    async def handle_deposit(
        self, player: Player, event: AstrMessageEvent, amount: int = 0
    ):
        """存入灵石"""
        if amount <= 0:
    ...
    async def handle_transactions(self, player: Player, event: AstrMessageEvent):
        ...
        for trans in transactions:
            trans_time = time.strftime(
                "%m-%d %H:%M", time.localtime(trans["created_at"])
            )
        ...
        msg_lines.extend(
            [
                "━━━━━━━━━━━━━━━",
                f"当前余额：{transactions[0]['balance_after']:,} 灵石"
                if transactions
                else "",
            ]
        )
    ...
    @player_required
    async def handle_breakthrough_loan(
        self, player: Player, event: AstrMessageEvent, amount: int = 0
    ):
        """申请突破贷款（用于购买破境丹）"""
        if amount <= 0:
