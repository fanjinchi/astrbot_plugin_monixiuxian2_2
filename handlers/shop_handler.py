...
class ShopHandler:
    ...
    def __init__(
        ...
        self.shop_manager_ids = {
            str(user_id) for user_id in access_control.get("SHOP_MANAGERS", [])
        }

    async def _ensure_pavilion_refreshed(
        self, pavilion_id: str, item_getter, count: int
    ) -> None:
        """确保阁楼已刷新"""
        ...
        if current_items:
            ...
            if updated:
                await self.db.update_shop_data(
                    pavilion_id, last_refresh_time, current_items
                )
        refresh_hours = self.config.get("PAVILION_REFRESH_HOURS", 6)
        if not current_items or self.shop_manager.should_refresh_shop(
            last_refresh_time, refresh_hours
        ):
    ...
    async def handle_pill_pavilion(self, event: AstrMessageEvent):
        ...
        await self._ensure_pavilion_refreshed(
            "pill_pavilion", self.shop_manager.get_pills_for_display, count
        )
        ...
        display = self.shop_manager.format_pavilion_display(
            "丹阁", items, refresh_hours, last_refresh
        )
    ...
    async def handle_weapon_pavilion(self, event: AstrMessageEvent):
        ...
        await self._ensure_pavilion_refreshed(
            "weapon_pavilion", self.shop_manager.get_weapons_for_display, count
        )
        ...
        refresh_hours = self.config.get("PAVILION_REFRESH_HOURS", 6)
        display = self.shop_manager.format_pavilion_display(
            "器阁", items, refresh_hours, last_refresh
        )
        yield event.plain_result(display)
    ...
    async def handle_treasure_pavilion(self, event: AstrMessageEvent):
        ...
        await self._ensure_pavilion_refreshed(
            "treasure_pavilion", self.shop_manager.get_all_items_for_display, count
        )
        ...
        refresh_hours = self.config.get("PAVILION_REFRESH_HOURS", 6)
        display = self.shop_manager.format_pavilion_display(
            "百宝阁", items, refresh_hours, last_refresh
        )
        yield event.plain_result(display)
    ...
    async def handle_buy(
        ...
        try:
            ...
            reserved, _, remaining = await self.db.decrement_shop_item_stock(
                pavilion_id, item_name, quantity, external_transaction=True
            )
            if not reserved:
                await self.db.conn.rollback()
            ...
            if item_type in [
                ...
                success, msg = await self.storage_ring_manager.store_item(
                    player, target_item["name"], quantity, external_transaction=True
                )
                if success:
                    type_name = {
                        "weapon": "武器",
                    ...
                    result_lines.append(
                        f"成功购买{type_name}【{target_item['name']}】x{quantity}，已存入储物戒。"
                    )
                else:
                    result_lines.append(
                ...
                if success:
                    result_lines.append(
                        f"成功购买材料【{target_item['name']}】x{quantity}，已存入储物戒。"
                    )
                else:
                    result_lines.append(
                        f"成功购买材料【{target_item['name']}】x{quantity}。"
                    )
                    result_lines.append(f"⚠️ 存入储物戒失败：{msg}")
            elif item_type == "功法":
                success, msg = await self.storage_ring_manager.store_item(
                ...
                if success:
                    result_lines.append(
                        f"成功购买功法【{target_item['name']}】x{quantity}，已存入储物戒。"
                    )
                else:
                    result_lines.append(
                        f"成功购买功法【{target_item['name']}】x{quantity}。"
                    )
                    result_lines.append(f"⚠️ 存入储物戒失败：{msg}")
            else:
                await self.db.conn.rollback()
