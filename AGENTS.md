...
AstrBot 插件（文字修仙游戏），Python 插件，无独立构建系统。本插件作为 AstrBot 的子目录运行，依赖 AstrBot 提供的运行环境与 API。

## 开发环境

- **插件目录**：`~/code/AstrBot/data/plugins/astrbot_plugin_monixiuxian2_2/`
- **宿主项目**：AstrBot 本体位于 `~/code/AstrBot/`，使用 **uv** 管理 Python 虚拟环境与依赖
- **Python 版本**：跟随 AstrBot，当前要求 `>=3.12`
- **依赖安装**：在 AstrBot 根目录执行 `uv sync`，插件额外依赖在 `requirements.txt`（当前仅 `Pillow>=9.0.0`），由 AstrBot 插件加载器在加载时自动安装
- **命令前缀**：本目录未配置独立 Python 解释器，应使用 AstrBot 的 uv 环境运行命令，例如：
  ...
  也可先 `cd ~/code/AstrBot` 再针对插件路径执行 `uv run ruff format data/plugins/astrbot_plugin_monixiuxian2_2/`。
...
5. **提交前使用 AstrBot 的 `ruff` 格式化并检查代码**（从插件目录执行 `uv run ruff format . && uv run ruff check .`，或在 AstrBot 根目录执行 `uv run ruff format data/plugins/astrbot_plugin_monixiuxian2_2/`）
