...
- 新建 `design_docs/content-design/armors.csv`：重甲/法袍两族 × 9 品级标杆件，列结构对齐 weapons.csv 约定（含 route_mult_ling/route_mult_ti），新增 `armor_family`、`bonus_hp` 列，删武器专属列。**品级体系对齐武器**（同一套 9 品级门槛 凡0/灵11/…/混元81，第一季实装凡~天，皇品起跨季外推），18 件标杆件全通用（向性件留变体扩展）。
- 确立对称 EHP 预算框架：重甲（护甲主、HP 辅）与法袍（HP 主、护甲辅）在同品级的等效生命倍率相等（M 序列：凡品 1.25 起每品 +0.025、皇品起封顶 1.35）；路线向性只通过 route_mult 对称系数表达。
- 确立防具供给曲线：由减伤目标反解 `A = K×r/(1−r)`，门槛级目标 重甲 r≈20%（带 18~22%）、法袍 r≈14%（带 12~16%，以武器护甲分品级递增 ~12.5% 基底重锚，原 6~10% 带数学不可达）；HP 辅量由对称 EHP 精确反解并经镜像 TTK ≥5 回合验收。
