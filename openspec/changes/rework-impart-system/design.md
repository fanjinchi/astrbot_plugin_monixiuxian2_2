...
- 迁移当前 `LATEST_DB_VERSION = 31`；v3.11.0 起不再向前兼容（历史逐版本迁移将在 v32 中移除）。
...
在 `handle_end_cultivation` 末尾对持有实例 `add_impart_value(effective_minutes // 15)`，一次事务完成；点数粒度读取 `cultivation_points_every_minutes`（默认 15，配置驱动，非硬编码）。备选：后台定时任务逐条+1——复杂度高、与"修炼时长"语义不一致、重启丢失。出关一次性结算与闭关时长强绑定，且天然处理上限（effective_minutes 封顶）。
...
**D5. 配置按类型分表 + 首版共享默认阈值。**
`impart_config.json` 重构为 `{ cultivation_points_every_minutes: 15, types: { common/sect/adventure/rift: { name, tiers: [{tier, impart_value_required, rewards}] } }, guardian: { enemy_group } }`。两个键均为运行时读取的配置驱动键：`cultivation_points_every_minutes` 控制出关点数粒度（player_handler 读取），`guardian.enemy_group` 指定守护 NPC 组 key（`EnemyManager.get_guardian_group_key()` 读取）。首版各类型复用原 20/40/60/80/100 五阶，结构独立、数值后续微调。旧配置缺失时 `_load_config_with_default` 给默认（`data/default_configs.py` 需补默认，否则空配置 → 全部跳过）。
备选：每类型独立文件——过度拆分；单文件 types 分组已满足。

**D5a. `common` 类型为遗留/兑底类型。**
三类获取途径仅产出 sect/adventure/rift，`common` 无新增获取途径（旧数据迁移已随 v3.11.0 不向前兼容重建移除，仅 GM 预置与兑底使用）。面板与帮助文案需向新玩家说明首条传承来自三类途径，避免出现无人可达的类型入口。
...
**D8. 数据库迁移 v32：不再向前兼容，统一重建为最新 schema。**
删除全部历史逐版本迁移（`_migrate_to_*`）与多套建表函数（`_create_all_tables_v1/v2/v21/v22`）；`_create_all_tables()` 成为唯一建表入口，直接生成 v32 最新 schema（含 `legacy_instances` 三表，`sect_id INTEGER` 类型统一）。`migrate()` 三分支：①fresh 建最新；②已注册 `MIGRATION_TASKS`（未来 v33+）按版本升序任务链逐版本升级（保留增量迁移能力）；③旧库（version < LATEST 且无注册任务）→ 警告后 `_drop_all_tables()` 全表 DROP → 重建 → `INSERT db_info.version=32`（旧数据清空，非保真）。回滚：重建后旧数据已清空，发布前需备份数据目录。

## Risks / Trade-offs

- **奖励 id 再次空转** → 实现阶段加配置存在性校验测试（tier 配置引用的心法/功法 id 必须存在于对应 JSON），并在重载后冒烟验证实际入包/入技能。
- **迁移破坏现有玩家进度** → v3.11.0 起明确不向前兼容：旧库升级将清空玩家数据（明示接受），发布前建议备份 `data/plugin_data/astrbot_plugin_monixiuxian2_2/`；迁移测试覆盖：重构后仅剩最新 schema（无残留旧表）、重建幂等、`MIGRATION_TASKS` 注册机制仍可执行后续版本任务。
...
1. 代码 + 配置 + 迁移重构（v32）一并发布；插件加载时 `migrate()` 自动执行：fresh 建最新 schema，旧库重建（数据清空）或走已注册的后续增量任务。
2. 3. 配置类变更（impart/adventure/rift/sect_factions/enemies）需重启生效；文档标注。
4. 回滚：整体回退代码与配置版本即可；旧库重建后原数据已清空——**升级前必须备份** `data/plugin_data/astrbot_plugin_monixiuxian2_2/`。
