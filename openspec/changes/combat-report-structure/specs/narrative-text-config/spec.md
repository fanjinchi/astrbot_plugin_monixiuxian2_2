...
### Requirement: 战报横幅代码所有

战报的结构横幅行（战斗开始、胜利、平局、战斗胶着平局、同归于尽）SHALL 由战斗引擎代码拼装，恢复 `☆━━━━ … ━━━━☆` 形态，其中胜利横幅的胜者名由代码插值。结构横幅 MUST NOT 依赖叙事配置——配置缺失/回退/文案迭代均不得让战报失去起止标志。`battle_opening`/`battle_victory`/`battle_draw`/`battle_draw_stalemate`/`battle_mutual_destruction` 五个叙事场景 SHALL 降级为横幅下方的文学描述行：配置池非空时随机取一条追加在横幅后；配置池缺失或为空时按既有空池规则回退内嵌最小默认描述行。描述行渲染 MUST NOT 抛错或阻断战报（沿用叙事渲染既有降级契约：format 失败时降级输出原始模板文本；该路径由加载时变量契约校验保证不可达）。

#### Scenario: 横幅恒定在场

- **WHEN** 任意一场战斗结算产出战报（含 config 中五个框架场景被清空或回退的极端情况）
- **THEN** 战报首行附近存在 `☆━━━━ 战斗开始 ━━━━☆`，结尾存在与结局对应的 `☆━━━━ … ━━━━☆` 横幅（胜利横幅含胜者名），无论叙事配置如何变化

#### Scenario: 文学描述行可选

- **WHEN** 某框架场景的文案池非空
- **THEN** 横幅后追加一条随机文学描述行；配置池缺失/为空时回退内嵌最小默认描述行；描述行渲染异常不抛错、不阻断战报，横幅始终在场
