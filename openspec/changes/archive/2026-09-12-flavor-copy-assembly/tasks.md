...
- [x] 7.7 本轮发现的遗留处置记录：
  - `-ju1` 成功面板标题与彩蛋粘连（`streak_bonus_msg` 是 D6 漏掉的同族插值点）——**已修**，换行责任落在 `core/breakthrough_manager.py` 插值侧（彩蛋可选 ⇒ 模板侧插 `\n` 会在连败 <3 时留空行，与 D6 对必然存在的 `{pity_msg}` 取模板侧的口径有意偏离），防回归钉为 `tests/test_breakthrough_manager.py::TestSuccessPanelStreakBonusLineBreak` 与 `functional_tests/cases/player/breakthrough-title-layout.json`
  - `-74n` copy_variants.csv 20 行吞进 `---` 条目分隔线——**已修**（design_docs 去分隔线 + 导入器 `_strip_separator` 兜底，commit `a288bba`）
