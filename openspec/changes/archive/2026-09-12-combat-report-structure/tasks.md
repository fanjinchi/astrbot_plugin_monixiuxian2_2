...
- [x] 5.2 分档覆盖（确定性设血口径，评审 Q2）：不再依赖 8000 血长局自然打低。残局档：GM 将防守方气血设为区间内值（如 4000/8000，ratio 0.5 ∈ (0.3,0.6]），一击后断言 `re:防守方名[^\n]{0,40}\d{1,3},\d{3}`（残局区间 hp≥1000 千分位必现）；濒死档：GM 设为低档内且**一击后仍存活**的值（如 2000，原则：设定值 ≤ 0.3×max_hp 且 > 单击伤害上限，具体以 fixture 实测为准），一击后断言 `re:防守方名[^\n]{0,40}\d{1,3}(,\d{3})?`（濒死区可出现 <1000 无逗号数字，不锚逗号）。千分位格式正确性由单测 2.5 钉，webtest 锚只验证「该档行出现 + 数字为代码格式化输出」；池句文本不进断言（防 -r0a/-xdl 同类文案锚静默翻红）
  - 2026-09-12 收尾任务【口径偏离，已实测校正】：任务书预设「GM 设血 → 开局 ratio=0.5」不成立——`managers/combat_manager.py:399` 的 `build_fighter_from_player` 中 `hp` 与 `max_hp` 同取 `total_attrs['hp']`，`core/gm_manager.py:713` 的 `_set_numeric_attr` 只 `setattr` 单一字段，故设血只会整段缩放，开局 ratio 恒为 1.0（已用 fixture 口径复核）。改用「长局必然穿档 + 结构锚」：保留 8000 血（2000 次仿真实测：中位 16 回合、范围 9–27，ratio 必数次穿 (0.3,0.6] 与 (0,0.3]），在 duel 用例追加两条分档断言——千分位形态 `re:测试玩家\d[^\n]{0,40}气血[^\n]{0,20}\d{1,3},\d{3}\s*点`（`pvp-basic-duel.json:113`）、无千分位形态 `re:测试玩家\d[^\n]{0,40}气血[^\n]{0,20}(?<![,\d])\d{1,3}\s*点`（`:126`，8000 血局下「带气血前缀的 <1000 数」只可能出自 ratio ≤ 0.3 的濒死档，证明该档真的渲染过）。两正则已本地仿真验证：命中两档池句、不命中间伤害行/面板行/`VS` 行/横幅/GM 回执（后者无「点」字）。spar 用例（双方各 1000 血、1–5 回合，仿真中位 2）不加分档断言，理由写在用例 note
  - 2026-09-12 评审 F1/F2 修（**blocker**，用例编排与统计口径）：两条分档 `combine` 断言各自**前置一条哨兵 `send`**（`#我的信息`，`pvp-basic-duel.json:106` / `:119` 的 `type` 行）——平台 runner 的 combine 窗口只在有新 out 消息到达时重新拼接匹配（`../astrbot_plugin_testplatform/cases/runner.py`：窗口初始化在步骤循环外、拼接与匹配均只在 `new_msgs` 非空分支内），游标 `after` 整用例单调递增，故 step 13 消费掉唯一那条战报消息后，后续 combine 断言窗口内无新消息 → 永不求值 → 超时 fail-fast（评审已用平台真实 runner + 真实用例探针验证前置哨兵后四条组合断言全绿；本轮另以 2000 次本地仿真复核：四条组合断言 2000/2000 命中、负向守护 2000/2000 干净）；同时订正统计口径：删除「约 40 回合」与「触发技必然命中」（实测 2000 次仿真中位 16 回合、范围 9–27；且 fixture 只写 `player_skills` 表，战斗装配读的是 `players.techniques`（fixture 恒为 `[]`）与武器配置（weapon 为空），本用例无触发技行）；新增 `tests/test_functional_case_assertions.py` 静态护栏钉住该根因类别（断言步骤前必须有新 `send`）
  - 2026-09-12 用例锚强度收紧（真机假绿发现）：两条分档断言的锚从「`气血` + 0–20 字任意内容」收到**分档行自身措辞**——千分位档 `re:测试玩家\d[^\n]{0,40}气血(?:还|只)剩\s*\d{1,3},\d{3}\s*点`（`functional_tests/cases/pvp/pvp-basic-duel.json:113`）、无千分位档 `re:测试玩家\d[^\n]{0,40}气血(?:还|只)剩\s*(?<![,\d])\d{1,3}\s*点`（`:126`），两档均不锚具体数值。触发原因：`combat.damage_crit` 变体 02（`config/narrative_config.json:309`）「招式刁钻，直取命门——{attacker_name} 一击中的，对方身形一顿，气血翻涌，连退两步才站稳。这一下 {final_damage} 点伤害，伤得不轻。」含「气血」且 20 字内接 `\d{1,3} 点`，故旧无千分位断言在**真机战报**（`functional_tests/results/2026-09-12_pvp-combat-report/messages/pvp-basic-duel__run752.json`）上命中 4 处——2 处真分档行（`测试玩家2 的气血只剩 686 点` / `测试玩家2 的气血只剩 189 点`）+ 2 处该伤害行（`…这一下 745 点` / `…这一下 752 点`）：分档功能整体回归时该断言仍可全绿（伤害行兜底），守护强度弱于原 note 声称。收紧后同一份战报只剩 2 处真分档行命中。两侧文案池 7 条变体（`remaining_hp_mid`×3 全写「气血还剩 {remaining_hp} 点」、`remaining_hp_low`×4 全写「气血只剩 {remaining_hp} 点」，`config/narrative_config.json:382-392`）与 `data/narrative_defaults/combat.py:74,78` 两条内嵌默认同形，故收紧不漏匹配；伤害行两档均不命中（`final_damage` 以裸 int 插值、无千分位，已在「池句 × hp 取值」矩阵上复核）。step 15 同法收紧（同不锚数值）：旧形态未在真机命中伤害行（伤害数无逗号），收紧只为与 step 17 口径一致、把守卫点收到分档行措辞上
- [x] 5.3 webtest 回归由用户手动发起后归档结果
  - 2026-09-12 真机回归（用户明确指示后执行；平台 v0.3.0 / 插件已重载 v3.17.1）：`pvp-basic-duel` 4/4（run 749，含**哨兵前提真机证实**：三次 `#我的信息` 各产出 1 条新 out 消息，窗口 `last_id` 依次推进）、`pvp-basic-spar` 绿（run 750）、**`pvp` tag 24/24**（runs 751-774，约 26 分钟）、PvE 抽查 `rift-encounters` 绿（run 775，两场真实 PvE 战斗同样渲染分档行）；结果归档 `functional_tests/results/2026-09-12_pvp-combat-report/`（24/24）与 `2026-09-12_pve-spotcheck/`，提交 `98b609f`
  - 2026-09-12 锚强度收紧后复跑：`pvp-basic-duel` run 776 绿（两档锚收紧到 `气血(?:还|只)剩` 紧邻数字，不再命中伤害行）；归档 `2026-09-12_pvp-duel-anchor-tighten/`，提交 `a260787`
...
- [x] 6.4 `openspec validate combat-report-structure --strict` 通过，任务全部勾选（待 5.3 用户手动回归完成后由主线归档）
  - 2026-09-12 主线归档：`openspec validate --strict` → valid；spec delta 合入 `openspec/specs/narrative-text-config/spec.md`（MODIFIED「叙事文案配置化载体」+ ADDED「战报结构行代码所有」「剩余气血行阈值分档」）；变更移至 `openspec/changes/archive/2026-09-12-combat-report-structure/`
