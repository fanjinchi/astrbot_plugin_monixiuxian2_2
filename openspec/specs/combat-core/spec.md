...
每次攻击的伤害 SHALL 按 Muxxu 式公式结算：`伤害 = floor((武器基础伤害 + 伤害属性 × 武器系数K) × 技能倍率 × 随机浮动 × (1 − 护甲减伤率))`，结果下限为 1。武器系数 K SHALL 随武器定位区分（大型武器 K 高、小型武器 K 低但基础伤害稳定）；空手 MUST 有保底基础伤害。护甲减伤为百分比（减伤率 = 护甲/(护甲+K)，K = armor_k_base + armor_k_level_coeff × 等级，总减伤 ≤ 40%），与技能减伤（格挡/减伤被动）乘算（见 attribute-numerics 的防御规约）。
