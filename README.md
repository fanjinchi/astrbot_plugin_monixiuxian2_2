...
### v3.8.0 - 新增配套：网页端测试平台（testplatform_plugin/）

**🧪 配套工具（不影响游戏玩法，主树零改动）**
- 新增 `testplatform_plugin/`：AstrBot 平台适配器（webtest）驱动的网页端测试平台，模拟玩家消息走真实消息管线（filter/Handler/白名单/数据库/定时任务全链路）
...
- 配套：CLI（scripts/test_platform_cli.py）、原生 JS 前端（webui/）、32 项单元测试；详见 `design_docs/test-platform.md`
