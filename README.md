...
- **双槽叙事形态落地**（openspec: flavor-copy-assembly）：突破成功/失败保命/死亡/复活、闭关开始/结算 6 个面板场景升级为「flavor 引子 + 机械面板」双槽渲染——面板模板单源（`panel` 键），引子按境界段/路线分桶随机；引子池为空（如化神后超出本赛季文案覆盖段）只出面板、不回退默认池，后续赛季补段位文案即可
...
### v3.16.1 - 叙事文案变体上线（战斗/修炼/历练事件）

**🎯 核心改动**

- **copy_variants 定稿文案入库**（season-1-tier1-copywriting 的导入侧落地，新增 `scripts/sync_copy_variants_to_config.py`）：24 个契约安全的短句场景（战斗日志为主）+ 13 个历练事件 desc_variants 分桶变体池写入 config，按玩家境界段/路线分轨随机，运行时原有的空池回落逻辑兜底
- **导入纪律**：① 变体未携带默认模板机械变量的场景（如突破失败面板的修为损失/连败保底、机缘掉落的物品名）不替换，保持原模板——19 个场景待"flavor 句 + 机械行"拼装设计（bd 3bt）；② 州条变体（186 条）待运行时世界状态选择轴（bd 3bt）；③ sect_duel/sect_trial 随五宗配置落地（bd n6o）
...
### v3.14.0 - 秘境遭遇系统：古阵谜题 & 可选挑战 & 传承应邀制

**🎯 核心改动**
- **秘境遭遇机制**：完成探索的基础结算（修为/灵石/事件/掉落）不变，结算后独立概率触发三类遭遇——古阵谜题/妖兽拦路/传承之地，互不互斥可同时触发；遭遇为内存 pending（默认 10 分钟惰性过期），通过「探索秘境」子命令响应：`探索秘境 破阵 <答案>` / `探索秘境 迎战` / `探索秘境 传承`
- **古阵谜题**：三个谜题族程序生成（五行破阵/洛书数阵/灵龟辨窟），题面碑文自带解题线索，零知识门槛；2 次机会，答对得额外掉落 roll+小额修为，答错/过期零惩罚
- **秘境 PvE 改可选挑战（行为变更）**：结算不再强制自动战斗；迎战胜利得敌人修为+额外掉落并计入师承任务链 win_pve 胜场，失败/平局气血降至 1 但不影响已结算的基础奖励，无视遭遇零损失
- **传承之地改应邀制**：秘境/历练结算命中传承概率后挂起传承遭遇而非立即开战，时限内应邀才挑战守护者（失败不致死、机缘消耗）；宗门宝库领取路径保持"领取即挑战"不变
- **秘境怪物池配置**：秘境条目新增 `enemy_group`（定向怪物组，不被普通 PvE 匹配命中）与 `encounter_rate`（触发率覆盖）字段，缺省回落按玩家境界匹配的全局池

**🧪 GM 测试工具（⚠️ 仅限测试实例）**
- `/修仙GM 触发秘境谜题|触发秘境妖兽|触发秘境传承 [目标]`：强制挂起对应遭遇供功能测试
- 测试秘境「试炼古境」（id 7，遭遇必触发 + 石傀儡测试怪组）随迁移播种，功能验证通过后拆除

---

### v3.13.0 - 事务原子性修复 & GM 测试工具 & 战斗/突破文案补漏外移

**🐛 数据一致性修复**
- **修复 9 处"假原子事务"**：银行存取/借款/还款、储物戒存入/取出/丢弃、商店购买、贷款逾期检查的事务块内嵌套了自带 commit 的数据层 helper，首个 commit 即提前终结事务释放写锁，原子性失效；统一改为 helper 支持 `commit` 参数、外层事务统一提交（沿用宗门路径既有模式）
- **修复商店购买装备/材料/功法类商品"成功路径丢货扣钱"**：`store_item` 在外部事务内不再重取并整行覆盖玩家对象，消除陈旧对象覆盖储物戒物品的问题
- 新增 21 例事务原子性回归测试（中途失败回滚、块级嵌套防护机器断言、购买成功路径断言）

**🧪 GM 测试工具（⚠️ 仅限测试实例）**
- `/修仙GM 时间快进 <秒>`：按显式字段清单前移冷却/到期时间（历练/修炼/战斗冷却/双修/悬赏/贷款到期/Boss/灵眼生成/传承冷却），多表改写包在单事务内，不可逆操作带确认
- `/修仙GM 清除全部冷却 [目标]`：一键归零指定玩家全部冷却与忙碌状态
- `/修仙GM 随机种子 <整数>|重置`：注入固定随机种子使概率类行为可确定性复现；不持久化，重置或重启恢复
- 全部复用既有 GM 白名单 + 审计日志

**📝 文案外移补漏（纯工程，文案零变更）**
- 突破机缘"储物戒已满"句与战斗域 7 处静态效果句式/回合头（反击/治疗/dot 附着/叠加上限拒绝/免死庇护/dot 侵蚀）补外移至 `config/narrative_config.json`（fortune/combat 节）
...
### v3.12.1 - 境界命名统一：体修不再使用独立十境词汇

**🎯 核心改动**
- **体修境界名与灵修统一**：废除锻体/铜皮/铁骨/钢筋/玄体/金刚/不灭/神体/肉身成圣/地仙体魄十境命名，体修同样显示 练气→筑基→金丹→元婴（及未来化神等），两路线共用同一境界表（`config/body_level_config.json` realms 与 `level_config.json` 一致）
- **剧情描述保留体修差异**：境界名相同，但突破/修炼文案仍用体修语系写出差异（"气血凝丹""丹破婴出"，而非"灵气凝丹"）
- 旧十境词仅保留为意象/内容名词系（如"铁布衫""铁骨丹""金刚寺"），排行、面板、突破文案中的境界名全部走新表

**🔧 其他**
- 同步世界观设定（world-bible §4.1：境界名统一，意象语系双轨）与剧情创作骨架（design_docs/剧情）
