...
class ImageGenerator:
    ...
    def __init__(self):
        ...
        if not self.has_pil:
            logger.warning(
                "【修仙插件】未检测到 Pillow 库，将无法生成图片卡片。请安装 pip install Pillow"
            )
    ...
    async def generate_user_info_card(
        ...
        try:
            ...
            return await asyncio.to_thread(
                self._draw_info_card_sync, user_id, detail_map
            )
    ...
    def _draw_info_card_sync(self, user_id: str, detail_map: Dict) -> BytesIO:
        ...
        for i, key in enumerate(right_keys):
            ...
            self._draw_status_line(
                img, key, str(val), 550, base_y + i * 103, font_36, color_text
            )
        ...
        for i, key in enumerate(base_keys):
            ...
            self._draw_wide_line(
                img, key, str(val), 100, base_list_y + i * 103, font_36, color_text
            )

        # 5. 宗门信息
        sect_y_header = (
            base_list_y + len(base_keys) * 103 + 50
        )  # 动态计算高度? 原版是硬编码
        sect_y_header = 1442  # 原版硬编码
        self._draw_section_header(
            img, "【宗门信息】", sect_y_header, font_40, color_text
        )
        ...
        for i, key in enumerate(sect_keys):
            val = detail_map.get(key, "无")
            self._draw_wide_line(
                img, key, str(val), 100, sect_y_list + i * 103, font_36, color_text
            )
