...
# 游戏全局参数默认值（各系统硬编码参数，供 config/game_config.json 缺失时物化）。
# 与装机文件同源：config_manager._load_config_with_default 用本默认创建文件；
# 两者的漂移由 tests/test_game_config_defaults.py 守卫（解析后必须深度相等）。
GAME_CONFIG = {
    "cultivation": {
        "max_cultivation_minutes": 1440,
        "comment": "闭关时长上限（分钟），24小时=1440分钟",
    },
    "combat": {
        "duel_cooldown": 300,
        "spar_cooldown": 60,
        ...
        "comment": "战斗冷却（秒）、暴击配置、概率上限与护甲公式参数。护甲公式：减伤率=护甲/(护甲+K)，K=armor_k_base+armor_k_level_coeff*等级；remaining_hp_mid_threshold/remaining_hp_low_threshold 为战报剩余气血行分档阈值（合法域 0<low<mid<1，非法值回退代码默认 0.3/0.6）",
        "status_stack_cap": 3,
        "remaining_hp_mid_threshold": 0.6,
        "remaining_hp_low_threshold": 0.3,
    },
    "skill_system": {
        "breakthrough_success_learn_rate": 0.2,
        "breakthrough_fail_learn_rate": 0.1,
        ...
        "growth_by_route": {
            "体修": {
                "hp_step": [17, 16, 15, 15],
                "growth_weights": [
                    {"damage": 0.55, "agility": 0.25, "speed": 0.2},
                    {"damage": 0.52, "agility": 0.26, "speed": 0.22},
                    {"damage": 0.49, "agility": 0.27, "speed": 0.24},
                    {"damage": 0.46, "agility": 0.28, "speed": 0.26},
                ],
            },
            "灵修": {
                "hp_step": [15, 16, 17, 17],
                "growth_weights": [
                    {"damage": 0.52, "agility": 0.22, "speed": 0.26},
                    {"damage": 0.55, "agility": 0.2, "speed": 0.25},
                    {"damage": 0.58, "agility": 0.18, "speed": 0.24},
                    {"damage": 0.62, "agility": 0.16, "speed": 0.22},
                ],
            },
            "comment": "按路线×大境界段的突破成长表（openspec: dual-route-identity，经 sim_route_matchup.py 校准定稿，配合灵修创角迅捷 10-18 的 C1 决策：练气段体修胜率约 61% 逐段收敛至元婴约 50%）。数组下标=大境界序号（0=练气…3=元婴），每段权重和恒为 1；升境界的那次突破按原境界段取表。灵修气血/伤害成长更高以收敛创角差距，体修保身法身份。缺失时回退上方全局 hp_growth_step/growth_weights",
        },
        "breakthrough_pity_step": 0.05,
        "breakthrough_pity_guarantee": 19,
        ...
        "comment": "技能系统：领悟概率、通用池概率、成长步长、连败保底",
    },
    "fortune": {
        "weapon_rate": 0.12,
        ...
        "pill_count_min": 1,
        "pill_count_max": 2,
        "comment": "突破机缘轮盘：武器/心法/丹药掉率互斥，余为空；level_window 为可掉落物品的最低境界偏移",
    },
    "pve": {
        "enabled": True,
        "difficulty_multiplier": 1.0,
        "comment": "PvE难度系数与总开关；关闭时禁用世界Boss/秘境等PvE战斗入口，历练不受影响",
    },
    "boss": {
        "enabled": True,
        "comment": "世界Boss系统总开关；关闭时停止Boss生成定时任务与相关指令",
    },
    "bank": {
        ...
        "breakthrough_loan_rate": 0.008,
        "breakthrough_loan_duration": 3,
        "comment": "银行配置：日利率、存款上限、贷款配置",
    },
    "dual_cultivation": {
        "cooldown": 3600,
        "request_expire": 300,
        "k_hours": 2,
        "realm_factor": "linear",
        "comment": "双修配置：冷却时间（秒）、请求过期时间（秒）。k_hours为定额收益等效闭关小时数；realm_factor为境界系数模式，可选 linear / power1.5 / power2",
    },
    "spirit_eye": {
        "spawn_interval": 7200,
        "types": {
    ...
    "rift": {
        ...
        "drop_tables": {
            ...
            "3": [
                ...
                {"name": "铁布衫", "weight": 20, "min": 1, "max": 1},
        ...
        "pill_drop_chance": {"1": 3, "2": 5, "3": 10},
        "comment": "秘境配置：探索时长（秒）、掉落表、丹药掉落概率",
    },
}
