# 功能测试结果：2026-09-11_full-regression

- 导出时间：2026-09-11T23:48:48
- 运行记录数：68
- 通过：67
- 失败/错误：1
- 不稳定：0
...
- pvp-weapon-trigger
- rift-encounters
- sect-adventure-event-marker
- sect-bounty-lifecycle
- sect-bounty-split
- sect-command-entry
- sect-construction
- sect-content-filter
- sect-default-visible
- sect-gm-set-contribution
- sect-gm-set-position
- sect-join-level-gate
- sect-leave-reclaim
- sect-master-chain-gm
- sect-master-task-chain
- sect-promotion
- sect-rejoin-retain
- sect-shop
- sect-shop-discount
- shop-buy-weapon-to-ring
- verify-char-creation

## 失败/错误用例

- breakthrough-title-layout (run 724, failed)
...
- breakthrough-fail-pity: pity_streak_bonus= 6, pity_hint_prose= 14（采样 2 次）
- breakthrough-guarantee: pity_guarantee= 6, streak_reward_prose= 6, fortune_drop_prose= 3（采样 2 次）
- breakthrough-title-layout: pity_guarantee= 2, streak_reward_prose= 2, fortune_drop_prose= 2（采样 1 次）
- pvp-basic-duel: round_header= 210, dodge_prose= 9（采样 2 次，最长回合=18）
- pvp-basic-spar: round_header= 30, dodge_prose= 3（采样 2 次，最长回合=3）
...
- pvp-heart-passive: round_header= 326, dodge_prose= 16（采样 1 次，最长回合=20）
...
- sect-adventure-event-marker: round_header= 24（采样 1 次，最长回合=1）
- sect-bounty-lifecycle: round_header= 66（采样 1 次，最长回合=2）
- sect-master-chain-gm: round_header= 16（采样 1 次，最长回合=2）
