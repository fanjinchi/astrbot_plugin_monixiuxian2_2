...
class CombatManager:
    ...
    def player_vs_player(
        ...
        combat_log.append(
            f"{player2.name}：HP {player2.hp}/{player2.max_hp}，ATK {player2.atk}"
        )
        combat_log.append("")
        ...
        while player1.hp > 0 and player2.hp > 0 and round_num < max_rounds:
            ...
            # 玩家1攻击
            is_crit1, damage1 = cls.calculate_turn_attack(
                player1.atk, player1.crit_rate
            )
            ...
            if is_crit1:
                combat_log.append(
                    f"{player1.name} 发起会心一击，造成 {damage1} 点伤害！"
                )
            else:
            ...
            # 玩家2攻击
            is_crit2, damage2 = cls.calculate_turn_attack(
                player2.atk, player2.crit_rate
            )
            ...
            if is_crit2:
                combat_log.append(
                    f"{player2.name} 发起会心一击，造成 {damage2} 点伤害！"
                )
            else:
    ...
    def player_vs_boss(cls, player: CombatStats, boss: CombatStats) -> Dict:
        ...
        while player.hp > 0 and boss.hp > 0 and round_num < max_rounds:
            ...
            # Boss攻击
            is_boss_crit, boss_damage = cls.calculate_turn_attack(
                boss.atk, 30
            )  # Boss固定30会心率
            ...
            if is_boss_crit:
                combat_log.append(
                    f"{boss.name} 发起会心一击，造成 {boss_damage} 点伤害！"
                )
            else:
